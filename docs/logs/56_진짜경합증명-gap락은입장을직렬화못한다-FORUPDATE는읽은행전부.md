...
## 7. 커밋 전 리뷰 후기 (같은 날) — 씨앗이 실증되고, 과장이 정정되고, 인덱스가 락 정밀도를 정한다
- **독립 리뷰가 씨앗을 실증** (log_38 패턴 재현): "enqueue 경로 같은 구조" 씨앗 → 리뷰 HIGH/MEDIUM으로 확인. JOIN 딸린 FOR UPDATE 4곳 잔존(WaitingService·PromotionService) + **승격 경로엔 오늘 제거한 "빈 슬롯 FOR UPDATE 후 INSERT" 안티패턴이 그대로** — 지금은 아웃박스 워커가 단일 스레드라 안 터질 뿐(가정에 기댄 안전).
- **정정**: 본문 §5의 "데드락 재발 없음"은 과장 — **예약 INSERT끼리만 참**. 넓은 waitings gap + 잔존 JOIN 락 + INSERT의 FK S락(부모 행)이 엮이면 생성↔대기신청 간 고리가 아직 가능.
- **gap 락이 어디 앉는지는 인덱스가 정한다** (§씨앗 "실행 계획이 곧 락 계획"의 즉시 회수): 잠금 단위 = "그 쿼리가 *스캔한* 인덱스 레코드". 슬롯 인덱스가 있으면 이웃 키들이 울타리가 돼 gap이 그 슬롯 틈으로 좁아지고, 못 타면 풀스캔 = 전부 + supremum. waitings의 UNIQUE는 member_id 선두라 슬롯 WHERE가 못 탐(leftmost prefix — 성 없이 이름으로 전화번호부 못 찾기) → **UNIQUE를 슬롯 선두로 재배열**(커밋 812ccf8a). 대가(member 단독 조회 인덱스 상실)는 학습자가 가격 매김: 슬롯당 5개 제한 → 테이블 항상 작음 → 수용. **둘 다 되면 심플한 쪽**(인덱스 1개) — log_55 결정 축 재사용.
- 씨앗(본문에만): **락 발자국 스윕** — JOIN FOR UPDATE 4곳 + 승격 경로 안티패턴을 한 패스로("카테고리는 한 번에"). 다음 arc 후보로 리뷰가 승격시킴.

## 다음
*arc '동시성 손끝 증명' **닫힘**(log_55~56). 규칙대로 다음 arc는 🧊 cold 메뉴([BACKLOG](../BACKLOG.md))에서 통째로 끌어올린다 — 후보: **락 발자국 스윕(§7, 리뷰 실증)**, reservation↔waiting 사이클 끊기+ArchUnit(구조/테스트), 재시도 정책(한 가족), JVM/런타임 등.*
