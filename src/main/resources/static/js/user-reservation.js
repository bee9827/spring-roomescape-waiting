  ...
  if (selectedDate && selectedThemeId && selectedTimeId) {
    ...
    fetch('/reservations', {
      ...
      body: JSON.stringify(reservationData)
        ...
        .then(response => {
          if (!response.ok) throw new Error('Reservation waiting.html failed');
          return response.json();
        })
        .then(data => {
          alert('Reservation waiting.html successful!');
          window.location.href = "/";
        })
        .catch(error => {
          alert("An error occurred while making the reservation waiting.html.");
          console.error(error);
        });
  } else {
    alert("Please select a date, theme, and time before making a reservation waiting.html.");
  }
}
